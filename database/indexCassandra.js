...
let updateDriver = driver => {
  ...
  const query = "UPDATE ks_drivers.drivers SET \
    updated_at = toTimestamp(now()), location = ?, availability = ?, activity = ? \
    WHERE driverId = ?";
  const params = [
    { x: driver.location.x, y: driver.location.y },
    driver.availability,
...
let countDriversByQuery = (column, callback) => {
  const startTime = new Date();
  query = `SELECT COUNT(${column}) FROM ks_drivers.drivers where ${column}=true`;
  client.execute(query, [], (err, results) => {
    if (err) {
      console.log(err);     
    } else {
      console.log(`Results: ${results.rows[0]['system.count(activity)'].low} in ${(new Date() - startTime) / 1000}s`);
    }
  });  
};

//countDriversByQuery("activity"); 

let findDriversByQuery = (column, callback) => {
  const startTime = new Date();
  query = `SELECT driverId FROM ks_drivers.drivers where ${column}=true LIMIT 10000000`;
  client.execute(query, [], (err, results) => {
    if (err) {
      console.log(err);     
    } else {
